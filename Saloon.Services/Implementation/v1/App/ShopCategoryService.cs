    ...
    public class ShopCategoryService : IShopCategoryService
    {
                    ...
                    using (var db = new SaloonDbContext())
                    {
                        using (var trans = db.Database.BeginTransaction())
                        {
                            try
                            {
                                ...
                                await db.ShopCategory.AddAsync(shop);
                        ...
                        using (var trans = db.Database.BeginTransaction())
                        {
                            try
                            {
                               ...
                               (x.Name == request.Name)
                                ...
                                var model = db.ShopCategory.FirstOrDefault(x => x.Id.Equals(request.Id));
        ...
        public async Task<ShopCommonVM> GetEdit(Guid id, Guid userId)
        {
            try
            {
                using (var db = new SaloonDbContext())
                {
                    var shop = await db.ShopCategory
        ...
        public async Task<bool> ControllActivation(Guid id, Guid userId)
        {
            try
            {
                ...
                using (var db = new SaloonDbContext())
                {
                    var model = db.ShopCategory.FirstOrDefault(x => x.Id.Equals(id));
